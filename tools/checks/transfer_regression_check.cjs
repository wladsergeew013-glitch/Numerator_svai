...
const { newImportPoints, reconcileCadImport } = load(path.join(root, 'frontend/src/utils/importPoints.ts'));
...
const { makeProjectCopy } = load(path.join(root, 'frontend/src/utils/projectFiles.ts'));
...
store.getState().setSelectedGroup('a');
assert.equal(store.getState().groupDetailsId, 'a', 'Open properties follow the selected group');
store.getState().closeGroupDetails();
assert.equal(store.getState().groupDetailsId, null);
store.getState().setSelectedGroup('b');
assert.equal(store.getState().groupDetailsId, null, 'Selecting a group does not open properties by itself');
assert.equal(createEmptyProject().viewSettings.dimOtherGroups, false);
store.getState().updateViewSettings({dimOtherGroups:true});
assert.equal(store.getState().project.viewSettings.dimOtherGroups, true);
const copiedProject = makeProjectCopy(store.getState().project);
assert.notEqual(copiedProject.project.id, store.getState().project.project.id, 'Save As creates a separate project identity');
assert.equal(copiedProject.project.fileName, null);
assert.equal(copiedProject.project.sourceFileName, null);
assert.equal(store.getState().project.project.fileName, 'saved-field.json', 'Save As does not change the open project');
...
const ready = point('linked', 0, 0, { meta: link, number: 73, groupId: 'group-1', locked: true, manualNumber: true });
const incomingCad = [point('incoming', 10, 5, {meta:{cadHandle:'a',cadDocument:'c:\\test.dwg',rawInsertionPoint:{x:10,y:5}}}),
  point('another', 0, 0, {meta:{cadHandle:'B',cadDocument:'C:/test.dwg'}})];
const reconciled = reconcileCadImport([ready], incomingCad);
assert.equal(reconciled.moved.length, 1);
assert.equal(reconciled.added.length, 1, 'Different handles at the same coordinate are different CAD objects');
assert.equal(reconciled.moved[0].id, 'linked');
assert.equal(reconciled.moved[0].number, 73);
assert.equal(reconciled.moved[0].groupId, 'group-1');
assert.equal(reconciled.moved[0].locked, true);
assert.equal(reconciled.moved[0].manualNumber, true);
assert.equal(reconciled.moved[0].syncState, 'moved');
const importedNewNumber = point('incoming-number', 10, 5, {meta:link, number:104, sourceNumber:'104'});
const unlockedReady = {...ready, locked:false};
assert.equal(reconcileCadImport([unlockedReady], [importedNewNumber]).moved[0].number, 73,
  'The default repeated import preserves existing numbers');
const overwrite = reconcileCadImport([unlockedReady], [importedNewNumber], {overwriteNumbers:true});
assert.equal(overwrite.moved[0].number, 104);
assert.equal(overwrite.renumbered.length, 1);
assert.equal(overwrite.renumbered[0].sourceNumber, '104');
assert.equal(overwrite.renumbered[0].manualNumber, true);
assert.equal(overwrite.renumbered[0].groupId, 'group-1');
assert.equal(reconcileCadImport([unlockedReady], [point('blank', 0, 0, {meta:link, number:null})], {overwriteNumbers:true}).renumbered.length, 0,
  'An empty model number cannot erase an existing number');
assert.equal(reconcileCadImport([ready], [importedNewNumber], {overwriteNumbers:true}).renumbered.length, 0);
assert.equal(reconcileCadImport([unlockedReady], [importedNewNumber], {overwriteNumbers:true, lockedGroupIds:new Set(['group-1'])}).renumbered.length, 0);
store.setState({project:{...createEmptyProject(),points:[ready]},selectedPointIds:[],history:[],redoStack:[]});
const savedBeforeSync = structuredClone(store.getState().project);
const sync = store.getState().syncCadImportedPoints('nanoCAD', incomingCad);
assert.equal(sync.moved.length, 1);
assert.equal(store.getState().project.points.length, 2);
assert.equal(store.getState().project.points[0].number, 73);
store.getState().undo();
assert.deepEqual(store.getState().project, savedBeforeSync, 'Undo restores original coordinates and existing numbering');
const rerun = reconcileCadImport([reconciled.moved[0], reconciled.added[0]], incomingCad);
assert.equal(rerun.moved.length, 0);
assert.equal(rerun.added.length, 0);
const oldWithoutHandle = point('existing-no-handle', 25, 30, { number: 81, groupId: 'group-1', meta: { source: 'nanocad_modelstudio' } });
const sameCadPoint = point('new-import', 25, 30, { meta: { source: 'nanocad_modelstudio', cadDocument: 'C:/test.dwg', cadHandle: 'C', objectName: 'Pile', numberParameter: 'NUMBER' } });
const linkedByCoordinate = reconcileCadImport([oldWithoutHandle], [sameCadPoint]);
assert.equal(linkedByCoordinate.added.length, 0);
assert.equal(linkedByCoordinate.linked.length, 1);
assert.equal(linkedByCoordinate.linked[0].id, oldWithoutHandle.id);
assert.equal(linkedByCoordinate.linked[0].number, 81);
assert.equal(linkedByCoordinate.linked[0].groupId, 'group-1');
assert.equal(linkedByCoordinate.linked[0].meta.cadHandle, 'C');
assert.equal(reconcileCadImport([oldWithoutHandle], [point('moved-without-handle', 35, 30, { meta: sameCadPoint.meta })]).added.length, 1);
const noHandleNewNumber = point('same-coordinate-number', 25, 30, { number:105, sourceNumber:'105' });
assert.equal(reconcileCadImport([oldWithoutHandle], [noHandleNewNumber]).renumbered.length, 0);
const noHandleOverwrite = reconcileCadImport([oldWithoutHandle], [noHandleNewNumber], {overwriteNumbers:true});
assert.equal(noHandleOverwrite.renumbered.length, 1);
assert.equal(noHandleOverwrite.renumbered[0].number, 105);
assert.equal(noHandleOverwrite.added.length, 0);
store.setState({project:{...createEmptyProject(), points:[oldWithoutHandle]}, history:[], redoStack:[]});
store.getState().syncCadImportedPoints('nanoCAD', [noHandleNewNumber], true);
assert.equal(store.getState().project.points[0].number, 105);
store.getState().undo();
assert.equal(store.getState().project.points[0].number, 81);
const editableCadPoint = point('editable-cad', 0, 0, { number: 73, groupId: 'group-1', meta: link });
store.setState({ project: { ...createEmptyProject(), points: [editableCadPoint] }, history: [], redoStack: [] });
store.getState().syncPointFromModel('editable-cad', 'position', { x: 5, y: 6, rawPosition: { x: 15, y: 16, z: 0 } });
assert.equal(store.getState().project.points[0].x, 5);
assert.equal(store.getState().project.points[0].number, 73);
assert.equal(store.getState().project.points[0].groupId, 'group-1');
store.getState().undo();
assert.equal(store.getState().project.points[0].x, 0);
