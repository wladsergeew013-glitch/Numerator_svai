...
rem ============================================================
rem 04_dump_project_tree.bat
rem Output goes to tools\out\project_tree.txt, not to tools root.
rem ============================================================
...
> "%OUT%" echo ============================================================
>>"%OUT%" echo PROJECT TREE - PILE NUMBERING APP
>>"%OUT%" echo ============================================================
>>"%OUT%" echo Root: %ROOT%
>>"%OUT%" echo Generated: %DATE% %TIME%
>>"%OUT%" echo.
>>"%OUT%" echo Folders overview:
if exist "%ROOT%\backend\"  >>"%OUT%" echo - backend\        - Python/FastAPI backend
if exist "%ROOT%\frontend\" >>"%OUT%" echo - frontend\       - React/TypeScript frontend
if exist "%ROOT%\tools\"    >>"%OUT%" echo - tools\          - helper scripts, checks and generated dumps
if exist "%ROOT%\projects\" >>"%OUT%" echo - projects\       - local .pilenum.json projects
>>"%OUT%" echo.
>>"%OUT%" echo Tree:
>>"%OUT%" echo ------------------------------------------------------------
>>"%OUT%" echo [D] .
...
if /I "%NAME:~0,7%"=="legacy_" (
  echo "%FULL%" | findstr /I "\\tools\\legacy_" >nul
  if not errorlevel 1 set "SKIP=1"
)
exit /b 0
...
set "NAME=%~1"
set "SKIP=0"
if /I "%NAME%"=="project_tree.txt" set "SKIP=1"
if /I "%NAME%"=="project_code_dump.txt" set "SKIP=1"
